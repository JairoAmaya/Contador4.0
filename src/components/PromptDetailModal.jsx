...
const PromptDetailModal = ({ promptData, onClose, onCopySuccess }) => {
  ...
  // 1. DETECCIÓN SEGURA DE VARIABLES
  useEffect(() => {
    // PROTECCIÓN: Si no hay promptData o prompt, no hacer nada
    ...
    const regex = /\[(.*?)\]/g;
    const found = promptData.prompt.match(regex); // Aquí era el error
  ...
  // 2. GENERADOR DE PROMPT FINAL SEGURO
  const finalPrompt = useMemo(() => {
    // PROTECCIÓN: Texto por defecto si viene vacío
    if (!promptData || !promptData.prompt) return "Error: No se encontró el contenido del prompt.";
  ...
  // Si no hay datos, no renderizar nada (o mostrar error)
        ...
        <div className="p-6 border-t border-slate-800 bg-[#1e293b] flex flex-col sm:flex-row gap-4 justify-between items-center">
          ...
          <button
            ...
            className={`w-full sm:w-auto order-1 sm:order-2 flex items-center justify-center gap-2 px-8 py-3 rounded-xl font-bold text-white shadow-lg transition-all transform active:scale-95 ${
              isCopied ? 'bg-green-600 hover:bg-green-700' : 'bg-blue-600 hover:bg-blue-500'
            }`}
          >
            {isCopied ? <><Check size={20} /> ¡Copiado!</> : <><Copy size={20} /> Copiar Prompt</>}
          </button>
        </div>
      </div>
    </div>
